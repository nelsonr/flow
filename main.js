...
function renderNodes (nodes, level = 0, input = null) {
    const treeHTML =
        nodes
            .map((node, index) => renderNode(node, level + 1, index, input))
            .flat()
            .join("");
...
const setupEvents = (state, nodes, input = null) => {
    nodes.forEach(node => {
        ...
        if (node.children.length > 0) {
            setupEvents(state, node.children, evaluateNode(node.code, input));
        }
